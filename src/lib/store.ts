...
export function addOrder(newOrder: Omit<Order, 'id' | 'createdAt' | 'status'>): Order {
  const order: Order = {
    ...newOrder,
...
export function updateOrderStatus(orderId: string, status: Order['status']): Order | undefined {
  ...
  if (index !== -1) {
    globalOrders[index].status = status;
    return globalOrders[index];
  }
  return undefined;
}

export function runAIDesignAnalysis(fileName: string): DesignAnalysisReport {
  // Generates intelligent, realistic design feedback based on UI guidelines
  const score = Math.floor(78 + Math.random() * 18);
  const layoutScore = Math.floor(80 + Math.random() * 15);
  const typographyScore = Math.floor(75 + Math.random() * 20);
  const contrastScore = Math.floor(82 + Math.random() * 14);
  const accessibilityScore = Math.floor(72 + Math.random() * 22);
