...
export const AuthModal: React.FC = () => {
    ...
    loadGsiScript().then(() => {
      ...
      g.accounts.id.initialize({
        ...
        callback: async (response: { credential?: string }) => {
          ...
          if (!res.ok) setError(res.error || t('auth_google_login_failed'));
          setSubmitting(false);
        },
      });
  ...
  }, [isOpen, isRegister, googleClientId, loginWithGoogle, t]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (isRegister && !name.trim()) { setError(t('auth_error_name_required')); return; }
    ...
    if (!res.ok) setError(res.error || t('auth_error_generic'));
    setSubmitting(false);
  };
      ...
      <div className="relative w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-apple-lg space-y-5">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-bold font-display text-slate-900">
            {isRegister ? t('auth_title_register') : t('auth_title_login')}
          </h2>
        ...
        {googleClientId ? (
          ...
          <div className="text-center text-[11px] text-slate-400 font-medium">
            {t('auth_google_config_note')}
          </div>
        )}

        <div className="relative flex items-center justify-center">
          <div className="border-t border-slate-200 w-full" />
          <span className="bg-white px-3 text-[10px] text-slate-400 font-bold uppercase">{t('auth_divider_email')}</span>
          <div className="border-t border-slate-200 w-full" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {isRegister && (
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700">{t('auth_label_name')}</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700">{t('auth_label_email')}</label>
            <div className="relative">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700">{t('auth_label_password')}</label>
            <div className="relative">
          ...
          >
            ...
            <span>{isRegister ? t('auth_submit_register') : t('auth_submit_login')}</span>
